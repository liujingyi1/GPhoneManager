...
public class FileIconLoader implements Callback {
    ...
    public static class FileId {
        ...
        public FileId(String path, long id, FileCategory cate, int width, int height) {
            mPath = path;
            mId = id;
            mCategory = cate;
            this.width = width;
    ...
    //add liujingyi start
    public boolean loadIcon(ImageView view, String path, long id, FileCategory cate, int width, int height) {
        boolean loaded = loadCachedIcon(view, path, cate);
        if (loaded) {
            mPendingRequests.remove(view);
        } else {
            FileId p = new FileId(path, id, cate, width, height);
            mPendingRequests.put(view, p);
            if (!mPaused) {
                // Send a request to start loading photos
                requestLoading();
        ...
        public boolean handleMessage(Message msg) {
            ...
            while (iterator.hasNext()) {
                ...
                if (holder != null && holder.state == ImageHolder.NEEDED) {
                    ...
                    switch (id.mCategory) {
                        case Apk:
                            Drawable icon = getApkIcon(mContext, id.mPath);
...
//                            holder.setImage(isVideo ? getVideoThumbnail(id.mId) : getImageThumbnail(id.mId));
                            if (id.width == 0) {
                            	...
                            	holder.setImage(isVideo ? getVideoThumbnail(id.mId) : getImageThumbnailByPath(id.mId, id.mPath, id.width, id.height));
                            }
        ...
        private Bitmap getImageThumbnail(long id) {
            return Images.Thumbnails.getThumbnail(mContext.getContentResolver(), id, Images.Thumbnails.MICRO_KIND, null);
        }
        
        private Bitmap getImageThumbnailByPath(long id, String path, int width, int height) {
        	
        	Bitmap bitmap = null;
        	
        	bitmap = getImageThumbnail(id);
        	if (bitmap != null) {
...
//        	bitmap = Video.Thumbnails.getThumbnail(mContext.getContentResolver(), id, Images.Thumbnails.MINI_KIND, options);
        	bitmap.recycle();
        	return thumbBitmap;
        }

        private Bitmap getVideoThumbnail(long id) {
            return Video.Thumbnails.getThumbnail(mContext.getContentResolver(), id, Images.Thumbnails.MICRO_KIND, null);
        }
    }
    ...
    public static Drawable getApkIcon(Context context, String apkPath) {
        ...
        if (info != null) {
            ...
            try {
                ...
                Log.e(LOADER_THREAD_NAME, e.toString());
            }
        }
        return null;
    }
