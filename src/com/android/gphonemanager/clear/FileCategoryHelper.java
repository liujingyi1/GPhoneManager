...
import com.android.gphonemanager.clear.MediaFileUtil.MediaFileType;
...
//    }
    ...
    private String buildSelectionByCategory(FileCategory cat) {
        ...
        switch (cat) {
            ...
            case Doc:
                ...
                break;
            case Zip:
                selection = "(" + FileColumns.MIME_TYPE + " == '" + sZipFileMimeType + "')";
                break;
            case Apk:
    ...
    public Cursor query(FileCategory fc, SortMethod sort) {
        ...
        String[] columns = new String[] {
                ...
                FileColumns.DATE_MODIFIED, FileColumns.DISPLAY_NAME
        };

        return mContext.getContentResolver().query(uri, columns, selection, null, sortOrder);
    }
    
    public int delete(FileCategory fileCategory, String path) {
        Uri uri = getContentUriByCategory(fileCategory);
        String selection = buildDeleteSelectionByCategory(fileCategory, path);

        if (uri == null) {
            Log.e(LOG_TAG, "invalid uri, category:" + fileCategory.name());
            return -1;
        }
        return mContext.getContentResolver().delete(uri, selection, null);
    }
    
    private String buildDeleteSelectionByCategory(FileCategory cat, String path) {
        String selection = null;
        switch (cat) {
            case Music:
            	selection = MediaStore.Audio.Media.DATA + "='" + path + "'";
                break;
            case Video:
            	selection = MediaStore.Video.Media.DATA + "='" + path + "'";
                break;
            case Picture:
            	selection = MediaStore.Images.Media.DATA + "='" + path + "'";
                break;
    ...
    public static FileCategory getCategoryFromPath(String path) {
        ...
        if (type != null) {
            if (MediaFileUtil.isAudioFileType(type.fileType)) return FileCategory.Music;
            if (MediaFileUtil.isVideoFileType(type.fileType)) return FileCategory.Video;
            if (MediaFileUtil.isImageFileType(type.fileType)) return FileCategory.Picture;
            if (sDocMimeTypesSet.contains(type.mimeType)) return FileCategory.Doc;
        }
